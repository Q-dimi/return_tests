...
function interval() {
        ...
        if(working_set[key].on === true) { 
            ...
            for(let i = 0; i < working_set[key].paths.length; i++) { 
                ...
                try {
                    ...
                    working_set[key].errors = working_set[key].errors.concat(errors); //loop(x) and attach path to errors
    ...
    keys.forEach((key, index) => {
        ...
        try {
            fs.writeFileSync('./errors.html', file_string); //working_set[key].html_file
        } catch (err) {
            console.log(err);
        }
