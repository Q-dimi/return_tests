  ...
  /* 
   ...
   Description: 
   strips every chosen type of function .html, .js, .ts files. Supports react.
   does not strip functions that are found inside strings("'`), single line comments and multline comments. 
   Includes the line number, filepath and function name for each function.
   Includes a list of function types like react functions, components, arrow functions, variable functions etc. All configurable.
   ...
   TODO:
   create a test for arrow functions and regular functions and end it...
   make sure to add var a = function() - just add another backtracking thing
   should work in all situations. should be more accurate than using 'window' if someone said something which i assume they did. window wont work for every file type like this will.
   ...
   add file types and check for in script out of script <script 
   only take .html, .js and .ts for now. not going to do other languages. skip over files that dont match these
   I started off not taking the declaration type into consideration. Maybe I can add this in. var, let, const. maybe i can add this in for all of them, make this seperate, then modify this
   monday 1-9 tuesday 3-9 friday 2-9 saturday 2-9 make sure to check schedule. also bro in the black. you are being tracked. yeah you who came to the back. video footage backtracking on that too bro. keep driving by je. you are doing great. Jeff St has passed by Alexander Eatmans house and has hired a man who came in all black to his work place on 2/27. Je has messages betweeen him and his ex suggesting psychological processes to manipulate and control Alex starting from first grade. Je is currently being tracked.
  ...
  */
  ...
  var file_type = '';
  var function_types = {
    regular: true, //backtrack on this for possible function name
    arrow: true, 
...
/*
recursing on every condition. Makes things easier to read. turning things on and off
*/
...
*/
 ...
 /* 
  enter into a string outside the function (if not in a single-line comment, and multi-line comment) whether im in or out of a string. dont need first two but makes things clear.
 */
 ...
 */
 ...
 /*
  COMBONATIONAL FUNCTIONAL SETS Enter into a regular function and start the build string. Above makes sure that the function below is built outside of a string, multiline comment and single line comment. So, I am getting the pure function when coming from the outside. When I do get a function, I ignore the outside, and focus on the inside. On the inside, I push every character, but dont run some conditions dependent on comments/strings. Once I hit this, im inside the function, and i simply just push every character and count brackets.
 */
 ...
 if(
  ...
  build_string = back_track_regular(drop_off_index_reg); //also make sure to back track the var a = function()
 ...
 */
 ...
 /* 
  enter into a string inside the function (if not or in a string, single-line comment, and multi-line comment) whether im in or out of a string. dont need first two conditions but helps make things clear.
 */
 ...
 */
 ...
 /* 
  end creating the function.. dont need last three conditions, but helps make things clear. the top condition should hit one time then going out of the function. The bottom should keep hitting. once pushed, reset everything and note that i am now out of a function. Going to need to add some additional things here.
 */
...
/*
 ARROW BT BEGINNING: ------------------------------------------------------ build the parameters for the arrow function. have to backtrack characters starting with ")"
*/
 ...
 */
 ...
 /*
  i begin traversing a string inside a function parameter set. dont need first two but helps make things clear.
 */
...
function get_arrow_parameter_function_name(bt_index_drop_off_function_name) { 
 ...
 /*
  if async exists add to the string... if not an async function, index remains the same for the next condition, which begins the function name
 */
...
/*
 REGULAR BT BEGINNING: ------------------------------------------------------ (change this) determining if regular function is async function. May need some additonal checks
*/

function back_track_regular(drop_off_index_reg) { 
  if(data.charAt(drop_off_index_reg) === 'c') { 
    if(take_five(drop_off_index_reg) === true) {
      return 'async function'; //backtrack on this and append... have to create an array and join characters.
    } else { 
      return 'function'; //backtrack on this and append
    }
  } else if(data.charAt(drop_off_index_reg) === '\s' || data.charAt(drop_off_index_reg) === ' ') {
