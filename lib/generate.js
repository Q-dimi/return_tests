...
function iterate_through_file_text(data_index) {
 ...
 if(
  data.charAt(data_index) === 'f' && //check if in string for function too
  ...
  function_string === false //make sure to only create function when in string
 ) { 
 ...
 if(
  data.charAt(data_index) === '=' && //arrow functions will not work with no brackets...ill try and fix it or just add something to stop it
 ...
 if(is_arrow === true && has_bracket === false && data.charAt(data_index) === '\n') { //arrow no bracket --- 
  push_function();
 } else if((opening_bracket.length === closing_bracket.length) && (opening_bracket.length > 0 && closing_bracket.length > 0)) { //arrow bracket function bracket
  push_function();
 }
...
function push_function() { //all function types now defined. arrow bracket, arrow no bracket, function bracket
