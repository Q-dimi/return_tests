 ...
 */
 ...
 if(
  (closing_bt_parentheses.length === opening_bt_parentheses.length) && //use count here and get rid of length
...
function get_arrow_parameter_function_name(bt_index_drop_off_function_name) { 
 ...
 /*
  if async exists add to the string... index remains the same for the next condition which is the function name, if not async.
 */
...
function enter_regular() { 
  in_function = true;
  build_string += 'function'; //back track and check if first letter going back is c, then run a minice set...
