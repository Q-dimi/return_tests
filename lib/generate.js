  ...
  /* 
   ...
   fix the functionfunction being built but works as of now for regular function... test out all the files in the folder and stripe all of them.
  ...
  */
  ...
  var in_comment_inside_function_single = false; //fix 2B
...
/*
 ...
 reversed it to avoid tripe quote error
*/
 ...
 if(
  ...
  in_string_outside_of_function_ = false;
  console.log('1B STRING MANY');
  return iterate_through_file_text(data_index);
 }

 /* 
  enter into a string outside the function (if not in a single-line comment, and multi-line comment) whether im in or out of a string, 
 */
 ...
 */
 ...
 if(
  (in_comment_type_outside_function_multi === true || in_comment_outside_function_single === true || in_string_outside_of_function_ === true) && 
  in_function === false
 ) {
  data_index = data_index + 1; 
  return iterate_through_file_text(data_index);
 ...
 */
 ...
 if(
  ...
  enter_regular();
  console.log('entering a regular function');
  return iterate_through_file_text(data_index); //if im adding 8 here, there should be function NOT functionfunction
 }
 ...
 */
  ...
  enter_arrow();
  console.log('entering an arrow function');
  return iterate_through_file_text(data_index);
 }
 ...
 */
  ...
  data_index = data_index + 1;
  console.log('nothing found outside the function and not in a string or comment, move next');
  return iterate_through_file_text(data_index);
 }
 ...
 */
 ...
 if(
  ...
  build_string += data.charAt(data_index);
  data_index = data_index + 1;
  console.log('2A STRING MANY');
  return iterate_through_file_text(data_index);
 }

 /*
  if not in a comment or string, keeping count of brackets so I know when to end the function... fix the brackets 
 */
 ...
 /* 
  end creating the function.. dont need last three conditions, but helps make things clear. the top condition should hit one time then going out of the function. The bottom should keep hitting.
 */
...
function get_arrow_parameter_function_name(bt_index_drop_off_function_name) { 
  ...
  */
  ...
  if(
   (bt_index_drop_off_found_first_character === true && (data.charAt(bt_index_drop_off_function_name) === '\s' || data.charAt(bt_index_drop_off_function_name) === ':')) ||
...
function push_function() {
 ...
 exported_functions += `
  { 
    ...
    function_called: {
    on: true,
    description: 'filepath is ${fp} AND line number is ${line_number}',
    parameters: [],
...
function enter_regular() { 
  ...
  build_string = back_track_async(drop_off_index_reg); //this is giving me functionfunction for whatever reason even though im moving forward 8 characters
  data_index = data_index + 8; //this should start the build string one character after n in function so it should only be test. but functionfunction for whatever reaason... if i get rid of it it works though... idk
  is_arrow = false;
}
