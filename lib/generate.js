...
function generate(folders, file_to_generate, unit) {
 ...
 for(let i = 0; i < unit_configuration.length; i++) { //im suing for stocking if you want to keep following me in the nissan bro. Its kind of funny its the same type, year and make i have. Maybe ill look at those cameras at the alcohol place tomorrow. n.m. you're probably going to go to jail if you dont stop. im not kidding. im preparing my documents and im calling some people (if i have to). If anything happens its you whos responsible.
  if(
