var fs = require('file-system');
  ...
  /* add the comments, run it, fix it, end it. done. rearrange some things near the top.. check for in function out of function in a better way if possible. then end it. once works, add react functions. 
  ...
  * @param {in_comment_type_outside_function_multi} tracking multiline comments outside function
 ...
 */
 ...
 /*
  ...
  if in a string and a match, i will already be at the next character index, so no need to increase here. if i wanted, i could check above right after pushing.
 */
 ...
 */
 ...
 /*
  Enter into a regular function and start the build string. Above makes sure that the function below is built outside of a string, multiline comment and single line comment. So, I am getting the pure function when coming from the outside. When I do get a function, I ignore the outside, and focus on the inside. On the inside, I push every character, but dont run some conditions dependent on comments/strings. For example, I do not execute the function if in a string or comment, and I do not push a bracket if in a string or comment. I also dont push the function if in a string or comment.
 */
 ...
 if(
  ...
  data.charAt(data_index+8) === '\n'
  //add all false strings here for inside function... 
 ) {
  enter_regular();
  return iterate_through_file_text(data_index);
 }
 ...
 */
 ...
 //use recurion actually and every time you are in something push the character as well. makes things easier to read.
 ...
 */
 ...
 if(
  ...
  ((opening_bracket.length === closing_bracket.length) && (opening_bracket.length > 0 && closing_bracket.length > 0)) && //use count
 ...
 */
 ...
 /*
  adding to the arrow function parameter character
 */
 ...
 */
 ...
 if(
  (closing_bt_parentheses.length === opening_bt_parentheses.length) && //use count
...
/*
 get the arrow function name by backtracking..... need to make sure i take async into consideration here
*/

function get_arrow_parameter_function_name(bt_index_drop_off_function_name) { 

  /*
   immediately take async into consideration when c is found here. just run a minice set
  */

  if(bt_af_is_async_check === false && data.charAt(bt_index_drop_off_function_name) === 'c') {
 ...
 /*
  if async exists add to the string... index remains the same for the next function name condition that is asked
 */
...
function is_async(bt_index_drop_off_function_name) { 
 ...
 if(
  ...
  bt_index_drop_off_function_name = bt_index_drop_off_function_name - 5; // https://www.youtube.com/watch?v=xdUpOao7ahw rip your doubt from your chest like you ripped my will. make you become weary for the rest of your life.
 }
...
function enter_regular() { 
  in_function = true;
  build_string += 'function'; //back track and check if first letter going back is c, then run a minice set.... like above turnary async check
