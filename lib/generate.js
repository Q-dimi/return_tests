var fs = require('file-system');
  ...
  * @param {f_is_async} used to turn on or off the condition that appends 'async'
  */
...
function get_arrow_parameter_function_name(bt_index_drop_off_function_name) { 
  ...
  /*
   immediately take async into consideration when c is found here. just do a minice set
  */

  if(f_is_async === false && data.charAt(bt_index_drop_off_function_name) === 'c') {
 ...
 /*
  async function b_t jawntrifier
 */
...
function enter_regular() { 
  //back track and check if first letter going back is c, then run a minice set.... like above
  in_function = true;
