 ...
 */
 ...
 if(
  data.charAt(data_index - 1) === ' ' && //other characters
...
function get_arrow_parameter_function_name(bt_index_drop_off_function_name) { 
  ...
  */
  ...
  if(
   ...
   (data.charAt(bt_index_drop_off_function_name) === ' ' || data.charAt(bt_index_drop_off_function_name) === ':') //look into this..may need to move left on : ...depends on stuff
  ) { 
   return;
  } else if(bt_index_drop_off_found_first_character === false && data.charAt(bt_index_drop_off_function_name) === ':') { 
    //backtrack object name 
    //return
    //-> wow = () => { }
    //-> a: wow = () => { }
    //-> a: () => { }
