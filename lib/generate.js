...
function back_track_parameters(bt_index) { 
 ...
 if((closing_bt_parentheses.length === opening_bt_parentheses.length) && (closing_bt_parentheses.length > 0 && opening_bt_parentheses.length > 0) && in_function === true) { //could use count
...
function get_arrow_parameter_function_name(bt_index_drop_off_function_name) { 
  ...
  //if first character found, unshift to function parameters.. if first found and first time, make sure to append an equals sign first... this is because i can only use one space.
