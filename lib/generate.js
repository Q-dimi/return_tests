  ...
  /* 
   ...
   make sure to add all possible start positions for each function. ignore constructor functions. or maybe add them
   make sure to recurse on if a function is invokable (); 
  */
...
function check_regular() { 
  if(
   (data.charAt(data_index-1) === '\s' || data.charAt(data_index-1) === '\n' || (data.charAt(data_index-1) === '=' && (data.charAt(data_index-2) === ' ' || data.charAt(data_index-2) === '\n' || data.charAt(data_index-2) === '\s')) || data.charAt(data_index-1) === ' ') && //|| (data.charAt(data_index-1) === '=' && (data.charAt(data_index-2) === ' ' || data.charAt(data_index-2) === '\n' || data.charAt(data_index-2) === '\s')) -- add this in for ever type (, +, -, ~, +, =, !
