...
// var function_recursive_count = 0; //used to track how any functions deep i am... using with in_function to compartmentalize being in and out of functions
...
function iterate_through_file_text(data_index) {
 ...
 //if i am in a string and not in a function
 ...
 //if the string ends and i am not in a function
 if(function_in_string.length > 1 && function_in_string[function_in_string.length - 1] === function_in_string[0] && in_function === false) { 
  function_in_string = [];
  function_string = false;
 }

 //if i am in a string and not in a function, continue going through file
 if(function_string === true && in_function === false) {
  data_index = data_index + 1; 
  return iterate_through_file_text(data_index);
 }

 //if i find a function, set myself inside a function and continue... this will hit the top then come to below here
 ...
 //if i find a function, set to function and continue... this will hit the top then come to below here.
 if(data.charAt(data_index) === '=' && data.charAt(data_index+1) === '>') {
  ...
  return iterate_through_file_text(data_index); //when i recursively find im in a function, 
 }

 //if im in a function and i find a string, set to string so that not to push a bracket
 ...
 //if i am in a function and i get out of a string, is_string off so i can push a bracket
 if(in_quotation_string.length > 1 && in_quotation_string[in_quotation_string.length - 1] === in_quotation_string[0] && in_function === true) { 
  in_quotation_string = [];
  in_string = false;
 }

 //if im not in a string and in the function, push to a bracket... so i can find out when to stop and append the function
 ...
 //if i am an arrow function and i dont have a bracket and its a new line, push the function. if i do have brackets and they match up, push the function. (this should append all inner functions as well evenly)
...
function back_track_parameters(bt_index) { 
 ...
 //return the "(a,b,c)"" set of parameters
...
function push_function() {
 ...
 build_string = '';
 has_bracket = false;
//  function_recursive_count = function_recursive_count - 1; 
 in_function = false;

//  if(function_recursive_count === 0) { 
//   in_function = false; 
//  }

//  if(function_recursive_count < 0) { 
//     throw new error('functions recursive count is less than zero')
//  }
