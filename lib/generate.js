...
//  }
 ...
 if(data.charAt(data_index) === 'f' && data.charAt(data_index+1) === 'u' && data.charAt(data_index+2) === 'n' && data.charAt(data_index+3) === 'c' && data.charAt(data_index+4) === 't' && data.charAt(data_index+5) === 'i' && data.charAt(data_index+6) === 'o' && data.charAt(data_index+7) === 'n') { // && space
  in_function = true;
 ...
 if(data.charAt(data_index) === '=' && data.charAt(data_index+1) === '>') { //and space
  in_function = true;
  bt_arrow_parameter_string = [];
...
function back_track_parameters(bt_index) { 
 ...
 if((closing_bt_parentheses.length === opening_bt_parentheses.length) && in_function === true) {
  //just before i go, i find that funciton name so i can append that as well, going backwards until i get past a definition const wow = () => { } 
...
//use an a-z array or regular expression until a space or new line is found. set an on off for first character found... once character found, set on. then new space or new line ends it.
function get_arrow_parameter_function_name(bt_index_drop_off_function_name) { 
  //unshift until space and get the fuck out. and just splatter everyone as you leave and dont give a fuck. and win bitch.
}
