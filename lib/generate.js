  ...
  /* 
   ...
   figure out all of the different types of functions to add and base everything off backtracking. you might only need one backtracking set for regular functions
   create a list of function definitions to place in the if else
   organize parameters by function type. each set containing on and off parameters for backtracking. ...arrow, regular
  */
  ...
  * @param {in_comment_inside_html_script_single} denoting if i am in a single line comment inside a script tag
  * @param {in_comment_type_inside_html_script_multi} tracking multiline comments inside a script tag
  */
  ...
  var in_comment_inside_html_script_single = false;
  var in_comment_type_inside_html_script_multi = false;
  ...
  fs.recurseSync(folders[i].folder, folders[i].files == 'all' ? null : folders[i].files, (filepath, relative, filename) => {
   ...
   if(filename) { 
    ...
    if(
     ...
     in_comment_inside_html_script_single = false;
     in_comment_type_inside_html_script_multi = false;
     opening_bracket = 0;
     closing_bracket = 0;
     build_string = '';
     is_arrow = false;
...
/*
recursing on every condition. Makes things easier to read. turning things on and off. When a definition for a function is found, I am backtracking to start the build string with the correct values.
*/
 ...
 */
 ...
 /*
  enter into an html script.. will need single and multi and string on here as well.
 */
 ...
 */  
 ...
 /* 
  Enter into a regular function (named and not named) and start the build string. I think i can use the backtracking array here for every regular function type. run conditions on the array.
 */
 ...
 if(
  ...
  build_string = back_track_regular(drop_off_index_reg); //also make sure to back track the var a = function() change this to the array and build it and whatever... have to also account for strings and comments here. will use if conditoins on array to turn some things off to run correct conditions.
...
/*
 recurse until the end of the script tag and update count
*/

function recurse_check_script(data_index_script_one) { 
...
function recurse_check_end_script(data_index_script_two) { 
