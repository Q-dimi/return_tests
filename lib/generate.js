  ...
  * @param {html_end_script_data_index} the index when > + 1 is found. Have to recurse in case of <     script>
  */
 ...
 for(let i = 0; i < folders.length; i++) {
  ...
  fs.recurseSync(folders[i].folder, folders[i].files == 'all' ? null : folders[i].files, (filepath, relative, filename) => {
   ...
   if(filename) { 
    ...
    file_type = filename.split(''); //backtrack this and lowercase then run the if else for the haters
 ...
 */
 ...
 /*
  check for html script. keep recursing if none found
 */
 ...
 */  
 ...
 /* 
  Enter into a regular function (named and not named) and start the build string. I think i can use the backtracking array here for every regular function type.
 */
 ...
 if(
  ...
  build_string = back_track_regular(drop_off_index_reg); //also make sure to back track the var a = function() change this to the array and build it and whatever
...
/*
 recurse until the end of the script '> + 1' -----------------------------------------------------
*/

function recurse_check_script(data_index) { 
