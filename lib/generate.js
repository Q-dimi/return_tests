 ...
 for(let i = 0; i < folders.length; i++) {
  ...
  fs.recurseSync(folders[i].folder, folders[i].files == 'all' ? null : folders[i].files, (filepath, relative, filename) => {
   ...
   if(filename) { 
    ...
    if(file_type[file_type.length - 1].toLowerCase() === 's' && file_type[file_type.length - 2].toLowerCase() === 't') { 
     file_type = 'typescript';
    } 

    else if(file_type[file_type.length - 1].toLowerCase() === 's' && file_type[file_type.length - 2].toLowerCase() === 'j') { 
     file_type = 'javascript';
    }

    else if(file_type[file_type.length - 1].toLowerCase() === 'l' && file_type[file_type.length - 2].toLowerCase() === 'm' && file_type[file_type.length - 3].toLowerCase() === 't' && file_type[file_type.length - 4].toLowerCase() === 'h') {
     file_type = 'html';
    }
    ...
    if(file_type === 'html' || file_type === 'javascript' || file_type === 'typescript') {
 ...
 */
 ...
 if(
  ...
  build_string = back_track_async(drop_off_index_reg); //also make sure to back track the var a = function()
 ...
 */
 ...
 /* 
  end creating the function.. dont need last three conditions, but helps make things clear. the top condition should hit one time then going out of the function. The bottom should keep hitting. once pushed, reset everything and note that i am now out of a function.
 */
...
function back_track_async(drop_off_index_reg) { 
  if(data.charAt(drop_off_index_reg) === 'c') { 
    if(take_five(drop_off_index_reg) === true) {
      return 'async function'; //backtrack on this and append
    } else { 
