...
//   function_recursive_count = function_recursive_count + 1;
  ...
  is_arrow = false;
  return iterate_through_file_text(data_index); //no return value on this... this is to save previous recursive state for in_function (return value should work if recursive set)
 }

 if(data.charAt(data_index) === '=' && data.charAt(data_index+1) === '>') {
//   function_recursive_count = function_recursive_count + 1; //might not need this actually... because im going off the brackets
  in_function = true;
  bt_arrow_parameter_string = [];
  ...
  return iterate_through_file_text(data_index); //no return value on this... this is to save previous recursive state for in_function -- or just keep a recursive count
 }
